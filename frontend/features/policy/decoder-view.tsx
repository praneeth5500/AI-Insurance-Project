...
import type { DecodedPolicy } from "@/lib/api/types";

/**
 * The decoder report (`docs/02_UX_UI_SPEC.md` section 12,
 * `docs/01_PRODUCT_SPEC.md` section 3.4).
 *
 * The UX spec describes a 65/35 split with policy Q&A on the right. Q&A is
 * Phase 13 and does not exist yet, so the report runs full width rather than
 * reserving a column for something that is not there — `CLAUDE.md` rule 8:
 * never make a UI claim the backend cannot support.
 *
 ...
 * limits up front.
 */
export function DecoderView({ decoded }: { decoded: DecodedPolicy }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <InlineAlert tone="info" title="What this is, and isn't">
        ...
        below links to the wording it came from — if something matters to you, read the wording and
